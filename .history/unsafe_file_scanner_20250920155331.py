...
class UnsafeFileScanner:
    ...
    def __init__(self, config_file: Optional[str] = None):
        ...
        if RULE_ENGINE_AVAILABLE:
            ...
            self.logger.info("Advanced rule engine initialized")
        else:
            self.logger.warning("Rule engine not available - install rule_engine.py")
        ...
        try:
            ...
            if self.rule_engine:
                ...
                for match in rule_matches:
                    if match.message not in all_issues:
                        all_issues.append(match.message)
            
            if not all_issues and not rule_matches:
                return None
            
            # Create unsafe file record
            risk_level = self.assess_risk_level(all_issues)
            
            # If rule engine found higher risk, use that
            if rule_matches:
                rule_risk_levels = [match.risk_level.value for match in rule_matches]
                if 'CRITICAL' in rule_risk_levels:
                    risk_level = 'CRITICAL'
                elif 'HIGH' in rule_risk_levels and risk_level not in ['CRITICAL']:
                    risk_level = 'HIGH'
                elif 'MEDIUM' in rule_risk_levels and risk_level not in ['CRITICAL', 'HIGH']:
                    risk_level = 'MEDIUM'
    ...
    def _save_csv_report(self, report: Dict, output_file: str) -> None:
        ...
        with open(output_file, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            writer.writerow(['Total Files Scanned', report['metadata']['total_files_scanned']])
            ...
            writer.writerow(['Scan Duration (seconds)', report['metadata']['scan_duration_seconds']])
            writer.writerow([])
...
<body>
    <div class="container">
        ...
        <div class="summary">
            ...
            <div class="stats">
                <div class="stat-card">
                    <div class="stat-number">{report['metadata']['total_files_scanned']}</div>
                ...
                <div class="stat-card">
                    <div class="stat-number">{report['metadata']['scan_duration_seconds']:.2f}s</div>
